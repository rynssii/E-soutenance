...
try {
    ...
    // Récupérer tous les thèmes autorisés de tous les professeurs avec les groupes associés
    $sql = "SELECT tp.id AS theme_id, tp.theme, c.name AS cycle_name, s.name AS specialite_name, u.fname AS prof_name,
                   g.student1, g.student2, g.student3, 
                   u1.fname AS student1_name, u2.fname AS student2_name, u3.fname AS student3_name
            FROM themeprof tp
            LEFT JOIN cycle c ON tp.cyclet = c.id 
            LEFT JOIN specialite s ON tp.specialt = s.id 
      ...
      <div class="container-fluid page-body-wrapper">
        ...
        <div class="main-panel">
          ...
          <div class="page-header">
            <h1 class="text-center"style="color:#B959B5 ;">Liste des thèmes autorisés</h1>
          </div>
            
          <h4 class="mb-5">Fixer une date de soutenance </h4>
        ...
        <table class="table table-hover">
            ...
            <?php foreach ($themes as $theme) { ?>
                <tr>
                    ...
                    <td><?php echo htmlspecialchars($theme['cycle_name']); ?></td>
                    <td><?php echo htmlspecialchars($theme['specialite_name']); ?></td>
                    <td><?php echo htmlspecialchars($theme['prof_name']); ?></td>
                    <td>
                        <?php echo htmlspecialchars($theme['student1_name']) . ', ' . htmlspecialchars($theme['student2_name']) . ', ' . htmlspecialchars($theme['student3_name']); ?>
                    </td>
                    <?php
                        // Vérifier si une date est déjà fixée pour ce thème
                        $sql = "SELECT date FROM soutenance WHERE theme = :theme_id";
                        $stmt = $conn->prepare($sql);
                        $stmt->bindParam(':theme_id', $theme['theme_id']);
                        $stmt->execute();
                        $date = $stmt->fetchColumn();
                    ?>
                    <td>
                        <input class="form-control" type="date" name="date_<?php echo $theme['theme_id']; ?>" id="date_<?php echo $theme['theme_id']; ?>" value="<?php echo $date ? htmlspecialchars($date) : ''; ?>">
                    </td>
                    <td><button class="btn btn-gradient-primary" type="submit" name="submit" value="<?php echo $theme['theme_id']; ?>">Envoyer</button></td>
                </tr>
            <?php } ?>
        </table>
    </form>
