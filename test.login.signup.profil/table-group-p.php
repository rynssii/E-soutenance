...
<title>Tableau des groupes et des thèmes</title>
<!-- plugins:css -->
      ...
      <div class="container-fluid page-body-wrapper">
        ...
        <div class="main-panel">
          <div class="content-wrapper">
            ...
            <h1 class="text-center"style="color:#B959B5 ;">Mes Groupes</h1>
            </div>
...
try {
    ...
    // Requête SQL pour récupérer le nom du groupe, le nom du thème, la spécialité et les IDs des étudiants
    $sql = "SELECT groupp.nomgroup, themeprof.theme, specialite.name AS specialite, groupp.student1, groupp.student2, groupp.student3 
            FROM groupp
            INNER JOIN themeprof ON groupp.theme = themeprof.id
            INNER JOIN specialite ON themeprof.specialt = specialite.id";
    $stmt = $conn->query($sql);
    ...
    if ($stmt->rowCount() > 0) {
        ...
        echo "<table>";
        echo "<tr><th>Nom du groupe</th><th>Nom du thème</th><th>Spécialité</th><th>Nom des étudiants</th></tr>";
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            echo "<tr>";
            echo "<td>".$row['nomgroup']."</td>";
            echo "<td>".$row['theme']."</td>";
            echo "<td>".$row['specialite']."</td>";

            // Requête SQL pour récupérer les noms complets des étudiants à partir de la table `users`
            $students = array();
            foreach (array('student1', 'student2', 'student3') as $student) {
                $studentID = $row[$student];
                if (!empty($studentID)) {
                    $sql_student = "SELECT fname, pname FROM users WHERE id = :studentID";
                    $stmt_student = $conn->prepare($sql_student);
                    $stmt_student->bindParam(':studentID', $studentID);
                    $stmt_student->execute();
                    $studentData = $stmt_student->fetch(PDO::FETCH_ASSOC);
                    $students[] = $studentData['fname'] . ' ' . $studentData['pname'];
                }
            }
