...
if (isset($_SESSION['id']) && isset($_SESSION['fname'])) {
    ...
    // Récupérer les groupes du professeur connecté
    try {
        $stmt = $conn->prepare("
            SELECT g.id, g.nomgroup 
            FROM groupp g 
            JOIN themeprof t ON g.theme = t.id 
            WHERE t.prof = ?
        ");
        $stmt->execute([$profId]);
        $groupes = $stmt->fetchAll(PDO::FETCH_ASSOC);
    } catch (PDOException $e) {
        echo "Erreur lors de la récupération des groupes : " . $e->getMessage();
        exit;
    }
?>
                ...
                <div class="col-12 grid-margin stretch-card">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form class="forms-sample" action="php/add-rdvc.php" method="post" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="time" class="form-control" name="heure" value="<?php echo (isset($_GET['heure'])) ? $_GET['heure'] : "" ?>" >
                                </div>
                                
                                <div class="form-group">
                                    <label for="lieu">Lieu *:</label>
                                    <input type="text" class="form-control" placeholder="exemple : salle2" name="lieu" value="<?php echo (isset($_GET['lieu'])) ? $_GET['lieu'] : "" ?>" >
                                </div>
                                
                                <div class="form-group">
                                    <label for="nmgroup">Groupe :</label>
                                    <select class="form-control" name="nmgroup">
                                        <?php foreach ($groupes as $groupe) { ?>
                                            <option value="<?php echo $groupe['id']; ?>"><?php echo $groupe['nomgroup']; ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
