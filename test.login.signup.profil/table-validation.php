...
// Supprimer le thème lorsque l'utilisateur clique sur "Refuser"
if(isset($_POST['delete_theme'])) {
    $theme_id = $_POST['theme_id'];

    try {
        $sql = "DELETE FROM themeprof WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$theme_id]);
        // Redirection pour actualiser la page après la suppression
...
// Récupération des thèmes de tous les professeurs avec leurs identifiants et leurs noms
try {
    $sql = "SELECT themeprof.id AS id_theme, themeprof.theme, users.id AS id_prof, users.fname, users.pname
            FROM themeprof
            INNER JOIN users ON themeprof.prof = users.id";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
      ...
      <div class="container-fluid page-body-wrapper">
        ...
        <div class="main-panel">
          ...
          <div class="page-header">
            <h1 class="text-center"style="color:#B959B5 ;">Liste des thémes</h1>
            </div>
        ...
        <tbody>
            <?php foreach ($themes as $theme): ?>
                <tr>
                    ...
                    <td><?php echo htmlspecialchars($theme['id_prof']); ?></td>
                    <td><?php echo htmlspecialchars($theme['fname'] . ' ' . $theme['pname']); ?></td>
                    <td>
                        <form method="post">
                            <input type="hidden" name="theme_id" value="<?php echo $theme['id_theme']; ?>">
                            <button class="btn btn-gradient-primary" type="submit" name="delete_theme">Refuser</button>
                        </form>
                    </td>
                </tr>
            <?php endforeach; ?>
